//! Domain policy for PartyEmail aggregate
//!
...
/// Domain policy for PartyEmail — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<PartyEmail>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type PartyEmailDomainPolicy = PermitAllPolicy<PartyEmail>;
