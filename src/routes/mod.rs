...
/// Create stateless routes for the Party module.
///
...
///
/// ```ignore
/// let module = PartyModule::builder().with_database(pool).build()?;
/// let routes = create_stateless_routes(&module);
/// let app = Router::new().merge(routes);
/// ```
pub fn create_stateless_routes(module: &crate::PartyModule) -> Router<()> {
    Router::new()
        .merge(create_party_routes(module.party_service.clone()))
        .merge(create_party_address_routes(module.party_address_service.clone()))
...
pub fn get_routes(module: &crate::PartyModule) -> Router<()> {
    Router::new()
...
/// Create stateful routes for the Party module.
///
...
pub fn create_combined_routes(module: &crate::PartyModule) -> Router<AppState> {
...
/// Get all routes (stateful) for the Party module.
///
/// This is a convenience function that wraps routes with API versioning.
pub fn get_routes_with_state(module: &crate::PartyModule) -> Router<AppState> {
    Router::new()
