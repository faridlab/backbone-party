...
//!
//! Tenancy (ADR-0029): the module is tenant-agnostic — no auth middleware, no scope binding.
//! Probes hit the guarded router directly on an UNDECORATED database: everything asserted here
//! is domain (validation, one-primary, kind coherence, atomic switch). Fence behavior is the
//! composing service's to prove; the non-owner RLS probe lives in the host's suite now.
