...
- "Add a new entity `Vendor`" → add `schema/models/vendor.model.yaml` → `metaphor make entity vendor` → migration/entity/service/handler/route wired automatically → register `vendor_service` in the `{Domain}Module` builder in `lib.rs`.
...
- "Change a column" → edit schema YAML, `metaphor migration alter <change>`, regenerate.
